    ...
    <>
      <section className="bg-midnight">
        <div className="container py-20 max-w-3xl">
          <span className="eyebrow animate-fade-in-up">Careers</span>
          <h1 className="text-white mb-6 animate-fade-in-up" style={{ animationDelay: '80ms' }}>
            Build the products, not just the backlog
          </h1>
      ...
      <section className="section-tight bg-gray-light">
        <div className="container grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {perks.map((perk, i) => (
            <Reveal key={perk} delay={i * 60}>
              <div className="card p-5 text-sm font-medium text-gray-dark transition-all duration-200 hover:shadow-md hover:-translate-y-0.5">
                {perk}
              </div>
            </Reveal>
          ))}
      ...
      <section className="section">
        <div className="container">
          ...
          {jobs.length === 0 ? (
            <Reveal className="card p-10 text-center max-w-xl">
            ...
            <div className="flex flex-col gap-4">
              {jobs.map((job, i) => (
                <Reveal key={job.id} delay={i * 60}>
                  <div className="card card-hover p-6 flex flex-col md:flex-row md:items-center gap-4 justify-between">
                    <div>
                    ...
                    <Link href="/contact" className="btn-outline whitespace-nowrap shrink-0">
                      Apply Now
                    </Link>
                  </div>
                </Reveal>
